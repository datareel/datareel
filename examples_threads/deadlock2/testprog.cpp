...
Test program demonstrating a dead lock where the application 
will attempt to recover from the dead lock.
...
// Thread's entry function
{
  ...
  if(mutex.MutexLock() != 0) { 
    ...
    if(mutex.GetMutexError() == gxMUTEX_OWNERDEAD_ERROR) {
      if(mutex.MakeConsistent() != 0) { // This thread now owns this lock
	cout << gxThreadMutexExceptionMessage(mutex.GetMutex()) << "\n" << flush;
	if(mutex.MutexUnlock() != 0) { // Unable to make the state consistent
	  cout << gxThreadMutexExceptionMessage(mutex.GetMutex()) << "\n" << flush;
	}
	has_mutex = 0;
      }
